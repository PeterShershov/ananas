...
function getComponentByProp(prop) {
	switch (getType(prop)) {
		...
		case "array":
			return prop.map((element, index) => <div key={element + index}>
				[
...
function componentsByObject(objectProp) {
	...
	for (const key of Object.keys(objectProp)) {
		...
		components.push(<div key={key}>{key}: {getComponentByProp(objectProp[key])}</div>);
	}
...
function generateStage(props) {
	...
	for (const key of Object.keys(demoProps)) {
		components.push(<div key={key}>{key}: {getComponentByProp(demoProps[key])}</div>);
	}
